...
内部 contribution 当前使用 JSON envelope 编码，并作为 `ResultSidecar.payload` 的 opaque bytes 传递。JSON schema 只由 storage profile producer 和最终 Proxy merger 理解，中间 reducer 不解析 payload。

实现约束包括：

- `schema_version = 1`；
- `bucket_schema = 1`；
- 单个 contribution 建议目标小于 8 KiB；
- 32 KiB 为软上限，用于发现 schema 膨胀；
- 64 KiB 为硬上限，超过后丢弃本次 contribution，业务结果继续成功；
- 单个通用 sidecar payload 同样限制为 64 KiB，单个结果转发的 sidecar payload 总预算为 1 MiB；
- 只序列化非零 operation；
...
```protobuf
message ResultSidecar {
  string type = 1;
  uint32 schema_version = 2;
  bytes payload = 3;
}

message ResultSidecars {
  repeated ResultSidecar items = 1;
  bool incomplete = 2;
}
...
schema_version = 1
payload = JSON contribution envelope
```

`ResultSidecar` 是通用内部承载层，不把 Storage Profile schema 固化进 Search/Query result。后续若增加其他诊断摘要，可以增加新的 `type`，而不需要让 topK、group-by、advanced Search 或 Query row reducer 理解新 payload。
...
5. 编码 JSON contribution；
6. 以 `type=storage_profile`、`schema_version=1` 附加到 internal result 的 `sidecars`。

### 12.3 中间 reduce 与 Proxy 合并

QueryNode 的 Search reduce、advanced Search reduce 和 Query pipeline 只调用通用 `resultsidecar.Merge`：

- 拼接 opaque sidecar item；
- 保留未知 `type` 和未知业务 payload；
- 限制单项为 64 KiB、总 payload 为 1 MiB；
- 对缺失、malformed 或超预算输入设置 `incomplete`；
- 不反序列化 JSON，不合并 histogram，不检查 contribution identity。

因此 topK merge、result slice、group-by reduce、advanced Search reduce 和 Query row merge 都不理解 Storage Profile 语义。

Proxy 在业务结果收集完成后，独立执行 Storage Profile merger：

1. 从 sidecar 集合中选择 `type=storage_profile`、`schema_version=1`；
2. 分别解码每个 contribution payload；
3. 丢弃 malformed 或未知版本 payload，并把 coverage 标为 partial/quantile incomplete；
4. 按 identity 去重；
5. 合并 count、bytes、distribution、coverage；
6. 交给 `NoopSummarySink`。

### 12.4 严格 fail-open

Storage Profile 不拥有 Search/Query 的返回错误。以下情况只增加 drop 指标、丢弃对应 sidecar，或把 coverage 标为 partial，不能让业务请求失败：

- snapshot 或 encode 失败；
- 未知 sidecar/profile schema；
- contribution 超过 64 KiB；
- sidecar 总预算超过 1 MiB；
- malformed payload；
- duplicate identity；
- histogram schema mismatch；
- 最终 merge 或 sink 失败。

当前 producer 的 `finishRequestStorageContribution` 在编码失败时返回 nil sidecar；中间 reducer 的 sidecar helper 不返回业务错误；Proxy 对每个 payload 独立解码，单个坏 payload 不再使其他 contribution 一并失效。
...
Profile 控制被封装在 `StorageProfileContext` 二级消息中；结果扩展被封装在通用 `ResultSidecars` 中。两者都减少未来继续扩展 Search/Retrieve 主消息顶层字段的需要。
...
- contribution 目标小于 8 KiB、软上限 32 KiB、硬上限 64 KiB；
- 单个结果的 sidecar payload 总预算为 1 MiB；
...
- Search/Query internal RPC 控制、opaque sidecar 转发与 Proxy 最终 contribution 合并；
...
| Opaque sidecar helper | `internal/util/resultsidecar/sidecar.go` |
