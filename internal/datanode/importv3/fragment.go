	...
	return Source{
		...
		Open: func(ctx context.Context) (storage.RecordReader, error) {
			return storage.NewImportFragmentRecordReader(ctx, storage.ImportFragmentReaderSpec{
				Path:     ref.GetPath(),
				Format:   storage.ImportFragmentFormatParquet,
				StartRow: 0,
				EndRow:   ref.GetRowCount(),
				Rows:     ref.GetRowCount(),
			}, temporarySchema,
...
// map. A fresh time.Now value is captured once per input Record batch and is
// not persisted in the task or plan, so retries are free to physically
