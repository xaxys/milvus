...
// TestGarbageCollector_recycleUnusedBinlogFiles_V3OrphanImportOwned pins the
// import ownership exemption: Import V3 registers a segment only at result
// acceptance, so an in-progress task's output files are unregistered and older
// than missingTolerance on slow imports. While the task record references the
// segment ID the orphan scan must skip them; segment IDs no task references
// are still recycled.
func TestGarbageCollector_recycleUnusedBinlogFiles_V3OrphanImportOwned(t *testing.T) {
	ctx := context.Background()
	...
	gc := newGarbageCollector(meta, &ServerHandler{}, GcOption{
		...
		missingTolerance: time.Hour,
		dropTolerance:    time.Hour * 24,
		// Segment 999 is owned by an in-progress ImportTaskV3; 997 is not.
		importV3SegmentIDs: func(context.Context) map[int64]struct{} {
			return map[int64]struct{}{999: {}}
		},
	})
	...
	insertPrefix := path.Join(cli.RootPath(), common.SegmentInsertLogPath)
	files := []*storage.ChunkObjectInfo{
		// expired unregistered files of the import-owned segment: skipped
		{FilePath: path.Join(insertPrefix, "100/10/999/_data/2003_uuid.parquet"), ModifyTime: expired},
		{FilePath: path.Join(insertPrefix, "100/10/999/_stats/bloom_filter.100/2002"), ModifyTime: expired},
		// expired unregistered files of a segment no task references: removed
		{FilePath: path.Join(insertPrefix, "100/10/997/_data/2005_uuid.parquet"), ModifyTime: expired},
	}
	...
	assert.ElementsMatch(t, []string{
		path.Join(insertPrefix, "100/10/997/_data/2005_uuid.parquet"),
	}, removedPaths)
}
