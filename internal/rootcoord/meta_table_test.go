...
func TestMetaTable(t *testing.T) {
	...
	t.Run("add collection", func(t *testing.T) {
		...
		collInfo.Partitions[0].PartitionCreatedTimestamp = ts

		err = mt.AddCollection(collInfo)
		assert.Nil(t, err)
	...
	t.Run("add partition", func(t *testing.T) {
		...
		coll, err := mt.GetCollectionByID(collID, 0)
		assert.Nil(t, err)
		assert.Equal(t, 1, len(coll.Partitions))

		err = mt.EnablePartition(collID, partID)
		assert.Nil(t, err)
		coll, err = mt.GetCollectionByID(collID, 0)
		assert.Nil(t, err)
	...
	t.Run("drop partition", func(t *testing.T) {
		defer wg.Done()
		ts := ftso()
		err := mt.DeletePartition(collID, partID, ts)
		assert.Nil(t, err)
	})
	...
	t.Run("add collection failed", func(t *testing.T) {
		...
		assert.Error(t, mt.AddCollection(collInfo))
	})
	...
	t.Run("delete collection failed", func(t *testing.T) {
		...
		ts := ftso()
		assert.Nil(t, mt.DeleteCollection(collInfo.CollectionID, ts))
	})
	...
	t.Run("delete partition failed", func(t *testing.T) {
		...
		ts = ftso()
		err = mt.DeletePartition(collInfo.CollectionID, -1, ts)
		assert.Nil(t, err)

		mockKV.save = func(key, value string, ts typeutil.Timestamp) error { return errors.New("mocked error") }
		err = mt.DeletePartition(collInfo.CollectionID, partID, ts)
		assert.Error(t, err)
...
func TestMetaWithTimestamp(t *testing.T) {
	...
	err = mt.AddCollection(collInfo)
	assert.Nil(t, err)
	err = mt.EnableCollection(collID1)
	assert.Nil(t, err)
	...
	collInfo = &model.Collection{
		...
		Name:         collName2,
		CreateTime:   t2,
		Partitions:   []*model.Partition{{PartitionID: partID2, PartitionName: partName2, PartitionCreatedTimestamp: ftso()}},
	}
	err = mt.AddCollection(collInfo)
	assert.Nil(t, err)
	err = mt.EnableCollection(collID2)
	assert.Nil(t, err)
	...
	_, err = mt.GetPartitionByName(collID1, partName1, 0)
	assert.Error(t, err)
	_, err = mt.GetPartitionByName(collID2, partName2, 0)
	assert.Error(t, err)
	err = mt.EnablePartition(collID1, partID1)
	assert.Nil(t, err)
	err = mt.EnablePartition(collID2, partID2)
	assert.Nil(t, err)
