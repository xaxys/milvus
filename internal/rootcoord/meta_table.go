...
func (mt *MetaTable) AddCollection(coll *model.Collection) error {
	mt.ddLock.Lock()
...
func (mt *MetaTable) EnableCollection(collID typeutil.UniqueID) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
...
func (mt *MetaTable) DisableCollection(collID typeutil.UniqueID) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
...
func (mt *MetaTable) EnablePartition(collID, partID typeutil.UniqueID) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
	...
	for _, partition := range coll.Partitions {
		if partition.PartitionID == partID {
			partition.Available = true
			break
		}
	}
...
func (mt *MetaTable) DisablePartition(collID, partID typeutil.UniqueID) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
...
func (mt *MetaTable) GetCollectionByID(collectionID typeutil.UniqueID, ts typeutil.Timestamp) (*model.Collection, error) {
	...
	if ts == 0 {
		...
		clone := col.Clone()
		clone.Partitions = funcutil.SliceFilter(clone.Partitions, func(partition *model.Partition) bool {
...
func (mt *MetaTable) DeletePartition(collID typeutil.UniqueID, partID typeutil.UniqueID, ts typeutil.Timestamp) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
...
func (mt *MetaTable) AddAlias(collectionAlias string, collectionName string, ts typeutil.Timestamp) error {
	...
	if aliasID, ok := mt.collAlias2ID[collectionAlias]; ok {
		if aliasID != id {
			collName := mt.collID2Meta[aliasID].Name
			return fmt.Errorf("another collection alias already exists, alias = %s, collection = %s", collectionAlias, collName)
		}
		return nil
	}
