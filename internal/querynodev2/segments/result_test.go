...
func (suite *ResultSuite) TestReduceSearchOnQueryNode_NonAdvanced() {
	...
	rEnc1.Sidecars = makeStorageProfileContribution(suite.T(), 1, "search-scope", 100)
	rEnc2, err := EncodeSearchResultData(ctx, srd2, nq, topK, metricType)
	suite.NoError(err)
	rEnc2.ScannedRemoteBytes = 333
	rEnc2.ScannedTotalBytes = 444
	rEnc2.Sidecars = makeStorageProfileContribution(suite.T(), 2, "search-scope", 200)
	...
	profile := mergedStorageProfileFromSidecars(suite.T(), out.GetSidecars())
