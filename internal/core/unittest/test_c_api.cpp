...
TEST(CApiTest, RetrieveTestWithExpr) {
    ...
    column_expr->field_offset_ = FieldOffset(1);
    column_expr->data_type_ = DataType::INT32;
    auto generic_value = std::make_unique<query::GenericValueImpl<int32_t>>();
    generic_value->data_type_ = DataType::INT32;
    generic_value->value_ = 0;
    auto term_expr = std::make_unique<query::TermExpr>();
    term_expr->child_ = std::move(column_expr);
    term_expr->terms_.emplace_back(std::move(generic_value));
...
TEST(CApiTest, Indexing_Expr_With_float_Predicate_Range_use_Compare_instead) {
    // insert data to segment
    constexpr auto TOPK = 5;
