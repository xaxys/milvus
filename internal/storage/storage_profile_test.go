...
func (s *profileObjectStorage) StatObject(context.Context, string, string) (int64, error) {
	return int64(len(s.content)), s.statErr
}
...
func TestChunkManagerProfilesExistNotFoundAsLogicalSuccess(t *testing.T) {
	root := t.TempDir()
	attribution := storageprofile.Attribution{
		Component:     "querynode",
		WorkloadClass: storageprofile.WorkloadClassRequestPath,
	...
	t.Run("local", func(t *testing.T) {
		...
		stat := recorder.Snapshot().Operations[storageprofile.StorageOperationStat]
		assert.Equal(t, uint64(1), stat.Count)
		assert.Equal(t, uint64(1), stat.Success)
		assert.Equal(t, uint64(0), stat.Failed)
		assert.Equal(t, uint64(0), stat.Errors[storageprofile.ErrorCategoryNotFound])
	})
	...
	t.Run("remote", func(t *testing.T) {
		...
		manager := &RemoteChunkManager{
			client:      &profileObjectStorage{statErr: merr.WrapErrIoKeyNotFound("missing")},
			bucketName:  "bucket",
			backendKind: storageprofile.BackendKindS3Compatible,
		}

		exists, err := manager.Exist(ctx, "missing")
		require.NoError(t, err)
		assert.False(t, exists)
