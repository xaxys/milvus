...
func beginProxyStorageProfile(ctx context.Context, workload storageprofile.WorkloadKind) (context.Context, *storageprofile.Scope, storageprofile.StorageProfileLevel, string) {
	...
	// Explicit request profiling is an internal-only signal until a public
	// authorization contract is designed for this diagnostic capability.
	authorized := contextutil.IsIntraClusterRequest(ctx)
	scope, decision := storageprofile.NewRequestScope(ctx, attribution, requested, explicit, authorized)
